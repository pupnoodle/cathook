...
int main()
{
  cat_ipc::shared_state state{};
  cat_ipc::command_s command{};
  pthread_mutex_init(&state.mutex, nullptr);

  command.payload_offset = cat_ipc::command_pool_size - 1;
  command.payload_size = 2;
  assert(!cat_ipc::command_payload(&state, command).has_value());

  command.payload_offset = 0;
  command.payload_size = 4;
  std::memcpy(state.pool, "abc", 4);
  assert(cat_ipc::command_payload(&state, command)->as_string() == "abc");

  std::memset(state.pool, 'x', 4);
  assert(!cat_ipc::command_payload(&state, command).has_value());
  assert(!cat_ipc::queue_command(&state, 0, cat_ipc::commands::execute_client_cmd,
    std::string(cat_ipc::command_payload_size, 'x')));
  assert(cat_ipc::allowed_console_command("cat_queue"));
  assert(!cat_ipc::allowed_console_command("cat_queue;quit"));
