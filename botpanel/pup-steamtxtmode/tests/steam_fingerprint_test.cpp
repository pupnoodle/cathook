...
int main()
{
  const bool supplied_seed = std::getenv("PUP_STEAM_TXTMODE_HARDWARE_SEED") != nullptr;
  if (!supplied_seed)
  {
    setenv("PUP_STEAM_TXTMODE_HARDWARE_SEED", "seed-a", 1);
    std::string seed_a;
    std::string seed_b;
  ...
  if (std::getenv("PUP_STEAM_TXTMODE_TEST_PRINT") != nullptr)
  {
