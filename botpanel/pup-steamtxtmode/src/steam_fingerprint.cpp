...
std::string seed()
{
  ...
  const char* explicit_seed = std::getenv("PUP_STEAM_TXTMODE_HARDWARE_SEED");
  if (explicit_seed != nullptr && explicit_seed[0] != '\0')
  {
    return explicit_seed;
  }
  if (env_flag("PUP_STEAM_TXTMODE_HARDWARE_RANDOMIZE", false))
  {
    return random_session;
  }
  const char* bot_id = std::getenv("PUP_BOT_ID");
...
{
  const int file_descriptor = static_cast<int>(syscall(SYS_memfd_create, "pup-steam-hardware", MFD_CLOEXEC));
...
{
  return is_enabled() && env_flag("PUP_STEAM_TXTMODE_SYNTHETIC_HARDWARE", false) && target_process();
}
