...
void set_config_value(config& settings, const std::string& key, const std::string& value)
{
  ...
  else std::cerr << "pupsteamtxtmode: ignoring unknown config key '" << key << "'\n";
}
...
std::string executable_directory(const char* argv0)
{
  ...
  const std::string fallback = argv0 == nullptr ? "pupsteamtxtmode" : argv0;
...
{
  return executable_directory(argv0) + "/pupsteamtxtmode.cfg";
}
...
{
  setenv("PUP_STEAM_TXTMODE", settings.textmode ? "1" : "0", 1);
  setenv("PUP_STEAM_TXTMODE_SYNTHETIC_HARDWARE", settings.synthetic_hardware ? "1" : "0", 1);
  setenv("PUP_STEAM_TXTMODE_HIDE_WINDOWS", settings.hide_windows ? "1" : "0", 1);
  setenv("PUP_STEAM_TXTMODE_DROP_DRAWS", settings.drop_draws ? "1" : "0", 1);
  setenv("PUP_STEAM_TXTMODE_TRIM_WEBHELPER", settings.trim_webhelper ? "1" : "0", 1);
  setenv("PUP_STEAM_TXTMODE_FRAME_INTERVAL_US", std::to_string(settings.frame_interval_us).c_str(), 1);
  if (!settings.hardware_seed.empty()) setenv("PUP_STEAM_TXTMODE_HARDWARE_SEED", settings.hardware_seed.c_str(), 1);
  else unsetenv("PUP_STEAM_TXTMODE_HARDWARE_SEED");
  setenv("PUP_STEAM_TXTMODE_HARDWARE_RANDOMIZE", settings.randomize_hardware ? "1" : "0", 1);
...
pid_t launch(const config& settings, const std::vector<std::string>& command)
{
  ...
  if (child < 0)
  {
    std::cerr << "pupsteamtxtmode: fork failed: " << std::strerror(errno) << '\n';
    return -1;
  }
  if (child == 0)
  {
    ...
    std::cerr << "pupsteamtxtmode: exec " << command[0] << " failed: " << std::strerror(errno) << '\n';
    _exit(127);
  }
  std::cout << "pupsteamtxtmode: launched " << command.front() << " pid=" << child << '\n';
  return child;
}
  ...
  const std::map<pid_t, std::string>& current)
{
  for (const auto& [pid, name] : current)
  {
    ...
    {
      std::cout << "pupsteamtxtmode: found " << name << " pid=" << pid
  ...
  for (const auto& [pid, name] : previous)
  {
    ...
    {
      std::cout << "pupsteamtxtmode: lost " << name << " pid=" << pid << '\n';
    }
  }
}
...
void usage(const char* program)
{
  std::cout << "Usage: " << program << " [--config PATH] [--once] [--help]\n"
    ...
    << "  with libpupsteamtxtmode.so in LD_PRELOAD.\n";
}
...
int main(int argc, char** argv)
{
  ...
  {
    std::cout << "pupsteamtxtmode: config not found, using built-in defaults: " << config_path << '\n';
  }
  const std::string default_preload = executable_directory(argv[0]) + "/libpupsteamtxtmode.so";
  ...
  pid_t steam_child = -1;
  pid_t tf2_child = -1;
  std::cout << "pupsteamtxtmode: watching " << names.size() << " process names\n";
  ...
  std::cout << "pupsteamtxtmode: stopped\n";
  return 0;
}
