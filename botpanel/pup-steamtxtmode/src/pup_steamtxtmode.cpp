...
#define PUP_STM_EXPORT extern "C" __attribute__((visibility("default")))

PUP_STM_EXPORT int XMapWindow(x_display* display, x_window window)
{
...
PUP_STM_EXPORT int XMapRaised(x_display* display, x_window window)
{
  static int (*real)(x_display*, x_window) = nullptr;
  if (should_hide_x11())
  {
...
PUP_STM_EXPORT int XRaiseWindow(x_display* display, x_window window)
{
  static int (*real)(x_display*, x_window) = nullptr;
  if (should_hide_x11())
  {
...
PUP_STM_EXPORT x_window XCreateWindow(
  x_display* display,
  x_window parent,
  int x,
  int y,
...
PUP_STM_EXPORT x_window XCreateSimpleWindow(
  x_display* display,
  x_window parent,
  int x,
  int y,
...
PUP_STM_EXPORT int XConfigureWindow(x_display* display, x_window window, unsigned int value_mask, void* values)
{
...
PUP_STM_EXPORT int XMoveResizeWindow(x_display* display, x_window window, int x, int y, unsigned int width, unsigned int height)
{
...
PUP_STM_EXPORT int XResizeWindow(x_display* display, x_window window, unsigned int width, unsigned int height)
{
...
PUP_STM_EXPORT int XIconifyWindow(x_display* display, x_window window, int screen)
{
...
PUP_STM_EXPORT void glClear(unsigned int mask)
{
...
PUP_STM_EXPORT void glDrawArrays(unsigned int mode, int first, int count)
{
...
PUP_STM_EXPORT void glDrawElements(unsigned int mode, int count, unsigned int type, const void* indices)
{
...
PUP_STM_EXPORT void glDrawRangeElements(unsigned int mode, unsigned int start, unsigned int end, int count, unsigned int type, const void* indices)
{
...
PUP_STM_EXPORT void glXSwapBuffers(void* dpy, x_drawable drawable)
{
...
PUP_STM_EXPORT unsigned int eglSwapBuffers(void* dpy, void* surface)
{
  sleep_loop_if(should_drop_gl());
...
PUP_STM_EXPORT void SDL_GL_SwapWindow(void* window)
{
  sleep_loop_if(should_drop_gl());
...
PUP_STM_EXPORT void* SDL_CreateWindow(const char* title, int x, int y, int w, int h, unsigned int flags)
{
...
PUP_STM_EXPORT void SDL_ShowWindow(void* window)
{
  static void (*real)(void*) = nullptr;
...
PUP_STM_EXPORT void SDL_RaiseWindow(void* window)
{
  static void (*real)(void*) = nullptr;
...
PUP_STM_EXPORT void SDL_SetWindowPosition(void* window, int x, int y)
{
...
PUP_STM_EXPORT void SDL_SetWindowSize(void* window, int w, int h)
{
  static void (*real)(void*, int, int) = nullptr;
...
PUP_STM_EXPORT void SDL_MaximizeWindow(void* window)
{
  static void (*real)(void*) = nullptr;
...
PUP_STM_EXPORT void SDL_RestoreWindow(void* window)
{
  static void (*real)(void*) = nullptr;
...
PUP_STM_EXPORT unsigned int eglSwapInterval(void* dpy, int interval)
{
...
PUP_STM_EXPORT void glXSwapIntervalEXT(void* dpy, x_drawable drawable, int interval)
{
...
PUP_STM_EXPORT int glXSwapIntervalSGI(int interval)
{
  static int (*real)(int) = nullptr;
  return next_symbol(real, "glXSwapIntervalSGI") != nullptr ? real(swap_interval(interval)) : 0;
}

PUP_STM_EXPORT int glXSwapIntervalMESA(unsigned int interval)
{
...
PUP_STM_EXPORT int SDL_GL_SetSwapInterval(int interval)
{
  static int (*real)(int) = nullptr;
...
PUP_STM_EXPORT gl_proc eglGetProcAddress(const char* name)
{
...
PUP_STM_EXPORT gl_proc glXGetProcAddressARB(const unsigned char* name)
{
...
PUP_STM_EXPORT gl_proc glXGetProcAddress(const unsigned char* name)
{
  static gl_proc (*real)(const unsigned char*) = nullptr;
  if (gl_proc override = proc_override(reinterpret_cast<const char*>(name)))
  {
...
PUP_STM_EXPORT int snd_pcm_open(void** pcm, const char* name, int stream, int mode)
{
...
__attribute__((constructor)) void pup_steamtxtmode_init()
{
  config& cfg = settings();
  cfg.log = env_flag("PUP_STM_LOG", true);
  cfg.disabled = !env_flag("PUP_STEAM_TXTMODE", true) || env_flag("PUP_STM_DISABLE", false);
  cfg.hide_x11 = env_flag("PUP_STM_HIDE_X11", env_flag("PUP_STEAM_TXTMODE_HIDE_WINDOWS", true));
  cfg.no_gl = env_flag("PUP_STM_NO_GL", env_flag("PUP_STEAM_TXTMODE_DROP_DRAWS", true));
  cfg.no_vsync = env_flag("PUP_STM_NO_VSYNC", true);
  cfg.no_audio = env_flag("PUP_STM_NO_AUDIO", true);
  cfg.webhelper_trim = env_flag("PUP_STM_WEBHELPER_TRIM", env_flag("PUP_STEAM_TXTMODE_TRIM_WEBHELPER", false));
  cfg.webhelper_single = env_flag("PUP_STM_WEBHELPER_SINGLE", false);
  cfg.loop_sleep = env_flag("PUP_STM_LOOP_SLEEP", true);
  cfg.loop_sleep_us = std::clamp(env_int("PUP_STM_LOOP_SLEEP_US", env_int("PUP_STEAM_TXTMODE_FRAME_INTERVAL_US", 100000)), 0, 1000000);

  if (cfg.disabled)
  {
    log_line("disabled via PUP_STM_DISABLE");
    return;
  }
