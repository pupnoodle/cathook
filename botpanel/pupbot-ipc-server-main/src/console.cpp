...
pup_ipc::shared_memory ipc_memory{};
pup_ipc::shared_state* ipc_state = nullptr;
...
{
  if (id >= pup_ipc::max_peers)
  {
    return true;
  }

  pup_ipc::try_scoped_lock lock{ipc_state};
  return !lock.locked() || !pup_ipc::peer_alive(ipc_state->peer_data[id]);
}
...
[[nodiscard]] auto kill_peer(const pup_ipc::peer_data_s& peer) -> bool
{
#if defined(SYS_pidfd_open) && defined(SYS_pidfd_send_signal)
  const auto pidfd = static_cast<int>(::syscall(SYS_pidfd_open, peer.pid, 0));
  if (pidfd >= 0)
  {
    if (pup_ipc::read_process_start_time(peer.pid) != peer.starttime)
    {
      ::close(pidfd);
      return false;
    }
...
[[nodiscard]] auto query_peer(unsigned int id) -> json
{
  ...
  json result{};
  if (!pup_ipc::peer_alive(ipc_state->peer_data[id]))
  {
    result["dead"] = true;
    return result;
  }
...
auto query(const json& args) -> json
{
  ...
  for (auto id = 0u; id < pup_ipc::max_peers; ++id)
  {
    if (skip_empty && peer_dead(id))
    {
      continue;
...
auto kill(const json& args) -> json
{
  ...
  if (id < 0 || id >= static_cast<int>(pup_ipc::max_peers))
  {
    throw std::out_of_range("peer out of range");
  }
  pup_ipc::try_scoped_lock lock{ipc_state};
...
auto connect(const json& args) -> json
{
  ...
  if (has_key(args, "server") && args["server"].get<std::string>() != "puphook_followbot_server")
  {
    throw std::runtime_error("custom ipc server names are not supported by this build");
  }

  ipc_memory = pup_ipc::shared_memory::open_client();
