...
const DEFAULT_SHARED_TF2_PATH = process.env.PUP_TF2_PATH || '/opt/steamapps/common/Team Fortress 2';
const VISIBLE_WINDOWS = process.env.PUP_VISIBLE_WINDOWS === '1';
const BOT_DISPLAY = process.env.DISPLAY || process.env.PUP_DEFAULT_DISPLAY || ':699';
const BOT_XAUTHORITY = VISIBLE_WINDOWS ? (process.env.XAUTHORITY || path.join(process.env.HOME || '', '.Xauthority')) : '';
const XPRA_LOG = process.env.PUP_XPRA_LOG || '/tmp/pupbot-xpra.log';
const TEXTMODE_GAME = process.env.PUP_TEXTMODE_GAME !== '0';
const BOT_TF2_OVERLAY_ENABLED = process.env.PUP_BOT_TF2_OVERLAY !== '0';
const STEAM_TXTMODE_ENABLED = process.env.PUP_STEAM_TXTMODE === '1';
const STEAM_TXTMODE_HIDE_WINDOWS = process.env.PUP_STEAM_TXTMODE_HIDE_WINDOWS === '0' ? '0' : '1';
const STEAM_TXTMODE_DROP_DRAWS = process.env.PUP_STEAM_TXTMODE_DROP_DRAWS === '0' ? '0' : '1';
const STEAM_TXTMODE_TRIM_WEBHELPER = process.env.PUP_STEAM_TXTMODE_TRIM_WEBHELPER === '1' ? '1' : '0';
const STEAM_TXTMODE_SYNTHETIC_HARDWARE = process.env.PUP_STEAM_TXTMODE_SYNTHETIC_HARDWARE === '0'
    ? '0' : (STEAM_TXTMODE_ENABLED ? '1' : '0');
const STEAM_TXTMODE_HARDWARE_RANDOMIZE = process.env.PUP_STEAM_TXTMODE_HARDWARE_RANDOMIZE === '1'
    || (process.env.PUP_STEAM_TXTMODE_HARDWARE_RANDOMIZE !== '0' && STEAM_TXTMODE_SYNTHETIC_HARDWARE === '1');
const STEAM_TXTMODE_HARDWARE_SEED = process.env.PUP_STEAM_TXTMODE_HARDWARE_SEED
    || (STEAM_TXTMODE_HARDWARE_RANDOMIZE
        ? `session-${crypto.randomBytes(16).toString('hex')}` : '');
const steam_txtmode_frame_interval_value = Number.parseInt(process.env.PUP_STEAM_TXTMODE_FRAME_INTERVAL_US || '100000', 10);
const STEAM_TXTMODE_FRAME_INTERVAL_US = Number.isSafeInteger(steam_txtmode_frame_interval_value)
    ...
    : '100000';
const STEAM_VGUI_TARGET_VERSION = process.env.PUP_STEAM_VGUI_TARGET_VERSION || '1689034492';
const STEAM_VGUI_REQUIRED = process.env.PUP_STEAM_VGUI === '1' || process.env.PUP_STEAM_VGUI !== '0';
const SKIP_DBUS_RUN_SESSION = process.env.PUP_SKIP_DBUS_RUN_SESSION === '1'
    || (process.env.PUP_SKIP_DBUS_RUN_SESSION !== '0' && TEXTMODE_GAME);
const GDB_CRASH_REPORTS = process.env.PUP_GDB_CRASH_REPORTS === '1'
    || (process.env.PUP_GDB_CRASH_REPORTS !== '0' && config.gdb_crash_reports === true);
...
const steam_window_options = process.env.PUP_STEAM_WINDOW_OPTIONS || steam_window_options_default;
const steam_shim_loop_sleep = process.env.PUP_STM_LOOP_SLEEP === '0' || process.env.PUP_STM_STEAM_LOOP_SLEEP === '0' ? '0' : '1';
const steam_shim_loop_sleep_us_value = Number.parseInt(process.env.PUP_STM_LOOP_SLEEP_US || process.env.PUP_STM_STEAM_LOOP_SLEEP_US || '100000', 10);
...
const steam_shim_webhelper_trim = process.env.PUP_STM_WEBHELPER_TRIM === '0' ? '0' : '1';
const steam_shim_webhelper_single = process.env.PUP_STM_WEBHELPER_SINGLE === '1' ? '1' : '0';
const game_window_options_default = VISIBLE_WINDOWS
    ? '-gl -sw -w 1280 -h 720'
    : (TEXTMODE_GAME ? '-silent -sw -w 1 -h 480' : '-gl -silent -sw -w 1 -h 480');
const GAME_WINDOW_OPTIONS = process.env.PUP_GAME_WINDOW_OPTIONS || game_window_options_default;
...
const textmode_allocator_assignments = TEXTMODE_GAME
    ...
    : '';
const SHARED_STEAM_ROOT = '/opt/pupbot-shared-steam';
const SHARED_STEAMAPPS = '/opt/steamapps';
const STEAM_OVERLAY = process.env.PUP_STEAM_OVERLAY === '1' || process.env.PUP_STEAM_OVERLAY !== '0';
const STEAM_OVERLAY_ROOT = process.env.PUP_STEAM_OVERLAY_DIR || '/opt/puphook/steam-overlays';
const STEAM_OVERLAY_PRIVATE_DIRS = ['appcache', 'config', 'logs', 'steamapps', 'steamapps_old', 'userdata'];
const PUPHOOK_ATTACH_DELAY_SECONDS = Number.parseInt(process.env.PUPHOOK_ATTACH_DELAY_SECONDS || '0', 10);
const TF2_LAUNCH_MODE = (process.env.PUP_TF2_LAUNCH_MODE || 'direct').toLowerCase();
const PER_BOT_X_DISPLAY = process.env.PUP_PER_BOT_X_DISPLAY === '1' || config.per_bot_x_display === true;
const PER_BOT_X_DISPLAY_BASE_VALUE = Number.parseInt(process.env.PUP_PER_BOT_X_DISPLAY_BASE || String(config.per_bot_x_display_base || '1000'), 10);
...
const PER_BOT_X_SCREEN = process.env.PUP_PER_BOT_X_SCREEN || process.env.PUP_X_SCREEN || '1x1x24';
const CHUNKED_X_DISPLAY_ENV = process.env.PUP_CHUNKED_X_DISPLAY;
...
const CHUNKED_X_DISPLAY_BASE = positive_integer(process.env.PUP_CHUNKED_X_DISPLAY_BASE || config.chunked_x_display_base, display_number_from_text(BOT_DISPLAY) || 699);
const CHUNKED_X_DISPLAY_BOTS_PER_DISPLAY = positive_integer(process.env.PUP_CHUNKED_X_DISPLAY_BOTS_PER_DISPLAY || config.chunked_x_display_bots_per_display, 25);
const CHUNKED_X_DISPLAY_MAX_DISPLAYS = positive_integer(process.env.PUP_CHUNKED_X_DISPLAY_MAX_DISPLAYS || config.chunked_x_display_max_displays, 12);
const CHUNKED_X_DISPLAY_MAX_CLIENTS = positive_integer(process.env.PUP_CHUNKED_X_DISPLAY_MAX_CLIENTS || process.env.PUP_XVFB_MAX_CLIENTS || config.chunked_x_display_max_clients, 512);
const CHUNKED_X_DISPLAY_PID_DIR = process.env.PUP_CHUNKED_X_DISPLAY_PID_DIR || '/tmp/pupbot-xvfb-pool';
const HEADLESS_STEAM_GRAPHICS = !VISIBLE_WINDOWS && process.env.PUP_STEAM_HEADLESS_GRAPHICS !== '0';
...
const STEAM_NOGRAPHICS_RENDERER_ENV = STEAM_TXTMODE_ENABLED
    ...
    : '';
const game_port_base_value = Number.parseInt(process.env.PUP_GAME_PORT_BASE || String(config.game_port_base || '30000'), 10);
const game_port_base = (Number.isSafeInteger(game_port_base_value) && game_port_base_value >= 1024) ? game_port_base_value : 30000;
const game_port_stride_value = Number.parseInt(process.env.PUP_GAME_PORT_STRIDE || String(config.game_port_stride || '10'), 10);
...
const LAUNCH_OPTIONS_STEAM = `env -u LD_PRELOAD firejail --ipc-namespace %DNS% %NETWORK% --noprofile --private="%HOME%" --private-tmp --private-dev --read-write=/opt/puphook/ipc --name=%JAILNAME% --env=PULSE_SERVER="unix:/tmp/pulse.sock" --env=DISPLAY=%DISPLAY% --env=XAUTHORITY=%XAUTHORITY% --env=TMPDIR=/tmp --env=TMP=/tmp --env=TEMP=/tmp --env=XDG_RUNTIME_DIR=/tmp/xdg-runtime --env=NO_AT_BRIDGE=1 --env=GTK_USE_PORTAL=0 --env=GIO_USE_VFS=local --env=PUP_SKIP_DBUS_RUN_SESSION=${SKIP_DBUS_RUN_SESSION ? '1' : '0'} ${HEADLESS_STEAM_GRAPHICS_FIREJAIL_ENV} --env=LD_LIBRARY_PATH=%STEAM_LD_LIBRARY_PATH% --env=LD_PRELOAD= --env=PUP_STEAM_TXTMODE_PRELOAD=%LD_PRELOAD% --env=PUP_STEAM_TXTMODE=${STEAM_TXTMODE_ENABLED ? '1' : '0'} --env=PUP_STEAM_TXTMODE_SYNTHETIC_HARDWARE=${STEAM_TXTMODE_ENABLED ? '1' : '0'} --env=PUP_STEAM_TXTMODE_HARDWARE_SEED=%HARDWARE_SEED% --env=PUP_STEAM_TXTMODE_HIDE_WINDOWS=${STEAM_TXTMODE_HIDE_WINDOWS} --env=PUP_STEAM_TXTMODE_DROP_DRAWS=${STEAM_TXTMODE_DROP_DRAWS} --env=PUP_STM_LOOP_SLEEP=%PUP_STM_LOOP_SLEEP% --env=PUP_STM_LOOP_SLEEP_US=%PUP_STM_LOOP_SLEEP_US% --env=PUP_STM_WEBHELPER_TRIM=%PUP_STM_WEBHELPER_TRIM% --env=PUP_STM_WEBHELPER_SINGLE=%PUP_STM_WEBHELPER_SINGLE% sh -lc 'mkdir -p "$XDG_RUNTIME_DIR"; chmod 700 "$XDG_RUNTIME_DIR"; if [ "$PUP_SKIP_DBUS_RUN_SESSION" = 1 ]; then exec env -u LD_PRELOAD DBUS_SESSION_BUS_ADDRESS="unix:path=/tmp/pup-disabled-dbus" "$@"; elif command -v dbus-run-session >/dev/null 2>&1; then exec dbus-run-session -- env -u LD_PRELOAD "$@"; else exec env -u LD_PRELOAD "$@"; fi' steam-session %STEAM% %STEAM_VGUI_ARG% ${steam_window_options} -login %LOGIN% %PASSWORD%`
const LAUNCH_OPTIONS_STEAM_RESET = 'env -u LD_PRELOAD firejail --ipc-namespace --net=none --noprofile --private="%HOME%" --private-dev --read-write=/opt/puphook/ipc --env=LD_LIBRARY_PATH=%STEAM_LD_LIBRARY_PATH% %STEAM% --reset'
const LAUNCH_OPTIONS_STEAM_WITH_HARDWARE = LAUNCH_OPTIONS_STEAM;
const LAUNCH_OPTIONS_GAME = `firejail --join=%JAILNAME% bash -c 'cd "%GAMEPATH%" && %RUNTIME_PREFIX% ${HEADLESS_STEAM_GRAPHICS_ASSIGNMENTS} ${textmode_allocator_assignments} SteamAppId=440 SteamGameId=440 SteamOverlayGameId=440 SteamEnv=1 PUPHOOK_ROOT="%PUPHOOK_ROOT%" PUPHOOK_ROOT_DIR="%PUPHOOK_ROOT%" PUPHOOK_AUTO_ATTACH=1 PUPHOOK_ATTACH_DELAY_SECONDS=%PUPHOOK_ATTACH_DELAY_SECONDS% PUP_BOT_ID="%BOT_ID%" PUP_BOT_NAME="%BOT_NAME%" PUP_STEAM_TXTMODE_HARDWARE_SEED="%HARDWARE_SEED%" PUP_STEAMID32=%STEAMID32% DBUS_SESSION_BUS_ADDRESS="unix:path=/tmp/pup-disabled-dbus" LD_PRELOAD=%LD_PRELOAD% DISPLAY=%DISPLAY% XAUTHORITY="%XAUTHORITY%" PULSE_SERVER="unix:/tmp/pulse.sock" %GAME_BINARY% -steam -game tf ${GAME_WINDOW_OPTIONS} -novid -nojoy -nomessagebox -nominidumps -nohltv -nobreakpad -noquicktime -precachefontchars -particles 1 -snoforceformat -softparticlesdefaultoff ${GAME_MODE_OPTIONS} -forcenovsync +volume 0 -noqueuedpacketprocessing -limitvsconst -nocrashdialog -noipx -threads 1 %GAME_PORT_OPTIONS% -nosteamcontroller -low +fps_max 30'`
const LAUNCH_OPTIONS_GAME_WITH_HARDWARE = LAUNCH_OPTIONS_GAME.replace(
    'PUP_BOT_ID="%BOT_ID%"',
    `PUP_BOT_ID="%BOT_ID%" PUP_STEAM_TXTMODE_SYNTHETIC_HARDWARE=${STEAM_TXTMODE_SYNTHETIC_HARDWARE}`,
);
...
const STEAM_CLIENT_INITIALIZED_GAME_DELAY_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAM_CLIENT_INITIALIZED_GAME_DELAY_SECONDS || String(steam_client_initialized_game_delay_default_seconds), 10);
...
const TIMEOUT_START_GAME = positive_integer(
    process.env.PUP_GAME_START_TIMEOUT_SECONDS || config.game_start_timeout_seconds,
    20
) * 1000;

const TIMEOUT_IPC_STATE = Number.parseInt(process.env.PUP_IPC_TIMEOUT_SECONDS || '300', 10) * 1000;
const ipc_heartbeat_stale_timeout = Number.parseInt(process.env.PUP_IPC_STALE_SECONDS || '45', 10) * 1000;
const ipc_identity_timeout = Number.parseInt(process.env.PUP_IPC_IDENTITY_TIMEOUT_SECONDS || '60', 10) * 1000;
const runtime_kill_grace_time = Number.parseInt(process.env.PUP_RUNTIME_KILL_GRACE_SECONDS || '8', 10) * 1000;
const AUTO_RESTART_MAX_FAILURES = positive_integer(process.env.PUP_AUTO_RESTART_MAX_FAILURES, 3);
const AUTO_RESTART_WINDOW_MS = positive_integer(process.env.PUP_AUTO_RESTART_WINDOW_SECONDS, 600) * 1000;
const AUTO_RESTART_STABLE_MS = positive_integer(process.env.PUP_AUTO_RESTART_STABLE_SECONDS, 60) * 1000;
const AUTO_RESTART_DELAYS_MS = [10000, 30000, 90000];

const TIMEOUT_STEAM_ASSUME_READY = Number.parseInt(process.env.PUP_STEAM_READY_SECONDS || '0', 10) * 1000;
const steam_logged_in_game_delay_default_seconds = 0;
const STEAM_LOGGED_IN_GAME_DELAY_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAM_LOGGED_IN_GAME_DELAY_SECONDS || String(steam_logged_in_game_delay_default_seconds), 10);
...
const STEAMWEBHELPER_CLEANUP_ENABLED = process.env.PUP_STEAMWEBHELPER_CLEANUP === '1' || config.steamwebhelper_cleanup === true;
const STEAMWEBHELPER_CLEANUP_DELAY_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAMWEBHELPER_CLEANUP_SECONDS || '10', 10);
...
const STEAM_LOGIN_UI_TIMEOUT_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAM_LOGIN_UI_TIMEOUT_SECONDS || '0', 10);
...
const BOT_START_DELAY_SECONDS_VALUE = Number.parseInt(process.env.PUP_BOT_START_DELAY_SECONDS || '30', 10);
const DELAY_START_TIME = (Number.isFinite(BOT_START_DELAY_SECONDS_VALUE) ? Math.max(0, BOT_START_DELAY_SECONDS_VALUE) : 30) * 1000;
const STEAM_BOOT_DELAY_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAM_BOOT_DELAY_SECONDS || String(config.steam_boot_delay_seconds || '8'), 10);
const STEAM_BOOT_DELAY = (Number.isFinite(STEAM_BOOT_DELAY_SECONDS_VALUE) ? Math.max(0, STEAM_BOOT_DELAY_SECONDS_VALUE) : 8) * 1000;
const STEAM_STARTUP_HARD_TIMEOUT_SECONDS_VALUE = Number.parseInt(process.env.PUP_STEAM_STARTUP_HARD_TIMEOUT_SECONDS || '900', 10);
...
let puphook_configs_ready = false;
...
function preload_value(primary_library) {
    const extra_preload = (process.env.STEAM_LD_PRELOAD || '')
        .split(':')
        .filter((entry) => entry && path.basename(entry) !== 'libpupsteamtxtmode.so');
    return [primary_library, ...extra_preload].filter(Boolean).join(':');
}

function steam_txtmode_library() {
    const library_name = 'libpupsteamtxtmode.so';
    const candidates = [
        process.env.PUP_STEAM_TXTMODE_LIBRARY,
        path.join(PUPHOOK_ROOT, 'botpanel/pup-steamtxtmode/bin/lib64/libpupsteamtxtmode.so'),
        path.join(PUPHOOK_ROOT, 'lib64/libpupsteamtxtmode.so'),
        '/usr/local/lib/libpupsteamtxtmode.so',
        '/usr/lib/libpupsteamtxtmode.so',
        '/usr/lib64/libpupsteamtxtmode.so',
        '/usr/lib/x86_64-linux-gnu/libpupsteamtxtmode.so',
        library_name
    ];
...
function steam_txtmode_library64() {
    const library_name = 'libpupsteamtxtmode.so';
    const candidates = [
        process.env.PUP_STEAM_TXTMODE_LIBRARY64,
        path.join(PUPHOOK_ROOT, 'botpanel/pup-steamtxtmode/bin/libx64/libpupsteamtxtmode.so'),
        '/usr/local/lib/libpupsteamtxtmode.so',
        '/usr/lib/x86_64-linux-gnu/libpupsteamtxtmode.so',
        '/usr/lib64/libpupsteamtxtmode.so',
        library_name
...
function steam_preload_value() {
    ...
    if (STEAM_TXTMODE_ENABLED) {
        const candidates = [
            process.env.PUP_STEAM_TXTMODE_LIBRARY,
            path.join(PUPHOOK_ROOT, 'botpanel/pup-steamtxtmode/bin/lib64/libpupsteamtxtmode.so'),
            path.resolve(__dirname, '../../pup-steamtxtmode/bin/lib64/libpupsteamtxtmode.so'),
            process.env.PUP_STEAM_TXTMODE_LIBRARY64,
            path.join(PUPHOOK_ROOT, 'botpanel/pup-steamtxtmode/bin/libx64/libpupsteamtxtmode.so'),
            path.resolve(__dirname, '../../pup-steamtxtmode/bin/libx64/libpupsteamtxtmode.so'),
            path.join(PUPHOOK_ROOT, 'lib64/libpupsteamtxtmode.so'),
        ];
        for (const candidate of candidates) {
...
function puphook_textmode_library() {
    const candidates = [
        path.join(PUPHOOK_ROOT, 'bin/libpuphooktextmode.so'),
        '/opt/puphook/bin/libpuphooktextmode.so',
        path.join(PUPHOOK_ROOT, 'bin/libpuphook-textmode.so'),
        '/opt/puphook/bin/libpuphook-textmode.so'
    ];
...
function puphook_graphical_library() {
    const candidates = [
        path.join(PUPHOOK_ROOT, 'bin/libpuphook.so'),
        '/opt/puphook/bin/libpuphook.so'
    ];
...
function puphook_game_library() {
    return TEXTMODE_GAME ? puphook_textmode_library() : puphook_graphical_library();
}
...
function bot_tf2_overlay_path(bot_home) {
    return path.join(bot_home, 'pup_tf2');
}
...
function max_steam_boots() {
    const value = Number.parseInt(process.env.PUP_STEAM_BOOT_CONCURRENCY || String(config.steam_boot_concurrency || '1'), 10);
    if (!Number.isSafeInteger(value) || value < 1)
        return 1;
...
const PROCESS_TABLE_CACHE_MS = Number.parseInt(process.env.PUP_PROCESS_TABLE_CACHE_MS || '1500', 10);
...
function bundled_puphook_configs_path() {
    return path.resolve(__dirname, '..', '..', '..', 'opt', 'puphook', 'configs');
}
...
function ensure_puphook_config_access(log) {
    if (puphook_configs_ready)
        return;

    const config_path = path.join(PUPHOOK_ROOT, 'configs');
    const log_path = path.join(PUPHOOK_ROOT, 'logs');
    fs.mkdirSync(config_path, { recursive: true });
    fs.mkdirSync(log_path, { recursive: true });

    const source_path = process.env.PUPHOOK_CONFIG_SOURCE || bundled_puphook_configs_path();
    ...
    if (copied_count > 0 && log)
        log(`Installed ${copied_count} bundled puphook config file(s) into ${config_path}`);

    puphook_configs_ready = true;
}
...
function get_default_network_interface() {
    if (process.env.PUPHOOK_NET_INTERFACE)
        return process.env.PUPHOOK_NET_INTERFACE;
...
const USER = {
    ...
    home: process.env.PUP_PANEL_USER_HOME || path.join('/home', process.env.SUDO_USER),
    interface: get_default_network_interface(),
    SUPPORTS_FJ_NET: process.env.PUP_USE_FIREJAIL_NET === '1'
};
if (!USER.interface) {
...
class Bot extends EventEmitter {
    constructor(botid) {
        ...
        ensure_puphook_config_access(this.log.bind(this));
    ...
    ensure_network_namespace() {
        ...
        if (this.network_namespace_failed)
            return false;
        if (fs.existsSync(`/var/run/netns/pupbotns${this.botid}`)) {
    ...
    hostSteamappsCandidates() {
        return unique_paths([
            process.env.PUP_STEAMAPPS_PATH,
            process.env.PUP_STEAM_ROOT ? path.join(process.env.PUP_STEAM_ROOT, 'steamapps') : null,
    ...
    sync_navmeshes() {
        ...
        const target_path = path.join(PUPHOOK_ROOT, 'navmeshes');
        var copied_count = 0;
        var source_count = 0;
    ...
    ensure_per_bot_x_display() {
        ...
        if (!PER_BOT_X_DISPLAY)
            return false;
        if (VISIBLE_WINDOWS) {
            this.log('Per-bot X display disabled because PUP_VISIBLE_WINDOWS=1; using shared display.');
            return false;
        }
        if (!xvfb_available()) {
    ...
    kill_stale_bot_runtime_processes() {
        ...
        const bot_namespace = escape_regex(`pupbotns${this.botid}`);
        const matchers = [
    ...
    spawnSteam() {
        ...
        if (!STEAM_TXTMODE_ENABLED)
            self.log('pup-steamtxtmode preload disabled (set PUP_STEAM_TXTMODE=1 to re-enable)');
        ...
        self.procFirejailSteam = child_process.spawn(([this.shouldResetSteam, this.shouldResetSteam = 0][0]
            ...
            .replace("%PUP_STM_LOOP_SLEEP%", shell_quote(steam_shim_loop_sleep))
            .replace("%PUP_STM_LOOP_SLEEP_US%", shell_quote(String(steam_shim_loop_sleep_us)))
            .replace("%PUP_STM_WEBHELPER_TRIM%", shell_quote(steam_shim_webhelper_trim))
            .replace("%PUP_STM_WEBHELPER_SINGLE%", shell_quote(steam_shim_webhelper_single))
            ...
            .replace("%NETWORK%", USER.SUPPORTS_FJ_NET ? `--net=${USER.interface}` : `--netns=pupbotns${this.botid}`)
            // Home folder
    ...
    spawnGame() {
        ...
        const source_library = puphook_game_library();
        ...
        if (TF2_LAUNCH_MODE === 'steam') {
            const steam_tf2_launch_options = [
                ...
                `SteamEnv=1`,
                `PUPHOOK_ROOT="${bash_double_quote_escape(PUPHOOK_ROOT)}"`,
                `PUPHOOK_ROOT_DIR="${bash_double_quote_escape(PUPHOOK_ROOT)}"`,
                `PUPHOOK_AUTO_ATTACH=1`,
                `PUPHOOK_ATTACH_DELAY_SECONDS=${PUPHOOK_ATTACH_DELAY_SECONDS}`,
                `PUP_BOT_ID="${bash_double_quote_escape(String(self.botid))}"`,
                `PUP_BOT_NAME="${bash_double_quote_escape(self.name)}"`,
                `PUP_STEAM_TXTMODE_HARDWARE_SEED="${bash_double_quote_escape(STEAM_TXTMODE_HARDWARE_SEED)}"`,
                `PUP_STEAMID32=${steamid32}`,
        ...
        self.log(`Launching TF2 mode=${TF2_LAUNCH_MODE} from ${game_launch_path} binary=${game_binary} source_library=${source_library} attach_delay_seconds=${PUPHOOK_ATTACH_DELAY_SECONDS} preload=${game_preload}`);
        self.procFirejailGame = child_process.spawn(game_launch_options.replace("%GAMEPATH%", bash_double_quote_escape(game_launch_path))
            ...
            .replace(/%PUPHOOK_ROOT%/g, bash_double_quote_escape(PUPHOOK_ROOT))
            .replace("%PUPHOOK_ATTACH_DELAY_SECONDS%", String(PUPHOOK_ATTACH_DELAY_SECONDS))
    ...
    removeGamePreloadLibrary(preload_library = this.gamePreloadLibrary) {
        ...
        try {
            ...
            this.log(`Removed temp puphook preload ${preload_library}`);
        } catch (error) {
            if (error.code === 'EACCES' || error.code === 'EPERM') {
                try {
                    ...
                    fs.unlinkSync(preload_library);
                    this.log(`Fixed permissions and removed temp puphook preload ${preload_library}`);
                    return;
                } catch (repair_error) {
                    this.log(`[ERROR] Failed to repair temp puphook preload permissions ${preload_library}: ${repair_error.message}`);
                }
            }
            if (error.code !== 'ENOENT')
                this.log(`[ERROR] Failed to remove temp puphook preload ${preload_library}: ${error.message}`);
        }
    }
            ...
            if (this.procFirejailSteam) {
                ...
                if (!this.isSteamWorking) {
                    ...
                    if (this.time_steamAssumeReady && time > this.time_steamAssumeReady) {
                        this.log('Steam readiness marker was not found; continuing because PUP_STEAM_READY_SECONDS fallback is enabled.');
                        this.markSteamReady();
                        return;
                    }
