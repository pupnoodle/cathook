...
class BotManager {
    constructor(cc) {
        ...
        this.update_budget_ms = Number.parseInt(process.env.PUP_MANAGER_UPDATE_BUDGET_MS || '12', 10);
        if (!Number.isSafeInteger(this.update_budget_ms) || this.update_budget_ms < 5)
            this.update_budget_ms = 12;
        this.update_batch_limit = Number.parseInt(process.env.PUP_MANAGER_UPDATE_BATCH || '8', 10);
        if (!Number.isSafeInteger(this.update_batch_limit) || this.update_batch_limit < 1)
            this.update_batch_limit = 8;
        this.update_slice_yield_ms = Number.parseInt(process.env.PUP_MANAGER_UPDATE_SLICE_YIELD_MS || '25', 10);
        ...
        this.quota_creation_batch_limit = Number.parseInt(process.env.PUP_BOT_CREATE_BATCH || '8', 10);
        ...
        this.snapshot_interval_ms = Number.parseInt(process.env.PUP_MANAGER_SNAPSHOT_INTERVAL_MS || '1000', 10);
    ...
    process_table_cache_ms() {
        ...
        if (count > 30)
            return 10000;
        return Number.parseInt(process.env.PUP_PROCESS_TABLE_CACHE_MS || '1500', 10);
    }
