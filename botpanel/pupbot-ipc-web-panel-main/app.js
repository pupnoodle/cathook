const PuphookConsole = require('./puphook');
...
const runtime_dir = process.env.PUP_RUNTIME_DIR || '/opt/puphook/run';

const PORT = Number.parseInt(process.env.PUP_IPC_PORT || '7655', 10);
...
try {
    const pid = npid.create('/tmp/ncat-puphook-webpanel.pid', true);
    pid.removeOnExit();
}
catch (error) {
...
fs.writeFileSync(path.join(runtime_dir, 'pup-webpanel-password'), basicAuth.password, { mode: 0o600 });
fs.chmodSync(path.join(runtime_dir, 'pup-webpanel-password'), 0o600);

const cc = new PuphookConsole();
...
function connect_ipc_console(generation) {
    ...
    cc.command('connect', {}, function (data) {
        ...
        if (data && data.status === 'success') {
            console.log('Connected to puphook IPC server');
            return;
        }

        const reason = data && data.error ? data.error : 'no response';
        console.log(`Failed to connect to puphook IPC server: ${reason}; retrying.`);
...
cc.on('exit', () => {
    cancel_ipc_console_connect();
    console.log('[!] puphook console disconnected; waiting for automatic respawn');
});
...
const HOST = process.env.PUP_IPC_BIND || '127.0.0.1';
