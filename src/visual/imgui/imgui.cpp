...
int ImTextCharFromUtf8(unsigned int *out_char, const char *in_text, const char *in_text_end)
{
    ...
    if ((*str & 0xf0) == 0xe0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        ...
        c = (uintptr_t) ((*str++ & 0x0f) << 12);
        if ((*str & 0xc0) != 0x80)
            return 3;
        c += (uintptr_t) ((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80)
            return 3;
        c += (*str++ & 0x3f);
        *out_char = c;
    ...
    if ((*str & 0xf8) == 0xf0)
    {
        *out_char = 0xFFFD; // will be invalid but not end of string
        ...
        c = (uintptr_t) ((*str++ & 0x07) << 18);
        if ((*str & 0xc0) != 0x80)
            return 4;
        c += (uintptr_t) ((*str++ & 0x3f) << 12);
        if ((*str & 0xc0) != 0x80)
            return 4;
        c += (uintptr_t) ((*str++ & 0x3f) << 6);
        if ((*str & 0xc0) != 0x80)
...
int ImTextStrToUtf8(char *buf, int buf_size, const ImWchar *in_text, const ImWchar *in_text_end)
{
    ...
    while (buf_out < buf_end - 1 && (!in_text_end || in_text < in_text_end) && *in_text)
    {
        unsigned int c = (uintptr_t) (*in_text++);
        if (c < 0x80)
            *buf_out++ = (char) c;
        else
