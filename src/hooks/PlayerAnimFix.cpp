...
static InitRoutine init(
    []()
    {
        auto ShouldInterpolate_signature = gSignatures.GetClientSignature(sigs::base_player_should_interpolate);
        if (ShouldInterpolate_signature)
            shouldinterpolate_detour.Init(ShouldInterpolate_signature, (void *) ShouldInterpolate_hook);

        auto FrameAdvance_signature = gSignatures.GetClientSignature(sigs::base_animating_frame_advance);
        if (FrameAdvance_signature)
