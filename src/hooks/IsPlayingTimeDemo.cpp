...
DEFINE_HOOKED_METHOD(IsPlayingTimeDemo, bool, void *_this)
{
    if (nolerp)
    {
        static uintptr_t wanted_addr = 0;
        static bool resolved         = false;
        if (!resolved)
        {
            resolved = true;
            auto call = gSignatures.GetClientSignature(sigs::interpolate_timedemo_call);
            if (call)
                wanted_addr = call + 6;
        }
        uintptr_t ret_addr = (uintptr_t) __builtin_return_address(0);
        if (wanted_addr && ret_addr == wanted_addr && CE_GOOD(LOCAL_E) && LOCAL_E->m_bAlivePlayer())
            return true;
    }
