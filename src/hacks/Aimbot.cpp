...
{
    if (class_is(projectile_size, CL_CLASS(CTFRocketLauncher), CL_CLASS(CTFRocketLauncher_Mortar), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFRocketLauncher_DirectHit), CL_CLASS(CTFPipebombLauncher), CL_CLASS(CTFGrenadeLauncher), CL_CLASS(CTFCannon)))
        return 4.0f;
    if (class_is(projectile_size, CL_CLASS(CTFFlareGun), CL_CLASS(CTFFlareGun_Revenge), CL_CLASS(CTFDRGPomson)))
        return 2.0f;
    if (class_is(projectile_size, CL_CLASS(CTFSyringeGun), CL_CLASS(CTFCompoundBow)))
        return 1.0f;
    return 3.0f;
}
...
bool projectileSpecialCases(AimbotTarget_t target_entity, int weapon_case)
{
    ...
    if (weapon_case == CL_CLASS(CTFCompoundBow))
    {
        bool release = false;
        if (autoshoot)
    ...
    if (weapon_case == CL_CLASS(CTFCannon))
    {
        bool release = false;
        if (autoshoot)
            current_user_cmd->buttons |= IN_ATTACK;
    ...
    if (weapon_case == CL_CLASS(CTFPipebombLauncher))
    {
