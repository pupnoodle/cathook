...
bool ShouldHitBuilding(CachedEntity *ent)
{
    ...
    if (autoupgrade_enabled)
    {
        ...
        int bid = ent->m_iClassID();
        if (bid == CL_CLASS(CObjectSentrygun))
        {
            if (!autoupgrade_sentry)
                return false;
            level = *autoupgrade_sentry_level;
        }
        else if (bid == CL_CLASS(CObjectDispenser))
        {
            if (!autoupgrade_dispenser)
                return false;
            level = *autoupgrade_dispenser_level;
        }
        else if (bid == CL_CLASS(CObjectTeleporter))
        {
...
CachedEntity *targetBuilding(bool priority)
{
    ...
    for (auto const &ent : entity_cache::valid_ents)
    {
        ...
        if (id == CL_CLASS(CObjectSentrygun))
        {
        ...
        else if (id == CL_CLASS(CObjectDispenser))
        {
        ...
        else if (id == CL_CLASS(CObjectTeleporter))
        {
...
{
    auto original = (CAM_CapYaw_t) CAM_CapYaw_detour.GetOriginalFunc();
    if (CE_INVALID(LOCAL_E) || !HasCondition<TFCond_Charging>(LOCAL_E))
        return original ? original(this_, fVal) : fVal;
