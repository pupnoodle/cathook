...
static bool ShouldReflect(CachedEntity *ent)
{
    ...
    int id = ent->m_iClassID();
    if (id == CL_CLASS(CTFGrenadePipebombProjectile))
    {
    ...
    if (id == CL_CLASS(CTFProjectile_Arrow))
        return *arrows;
    if (class_is(id, CL_CLASS(CTFProjectile_Jar), CL_CLASS(CTFProjectile_JarMilk)))
        return *jars;
    if (id == CL_CLASS(CTFProjectile_HealingBolt))
        return *healingbolts;
    if (id == CL_CLASS(CTFProjectile_Rocket))
        return *rockets;
    if (id == CL_CLASS(CTFProjectile_SentryRocket))
        return *sentryrockets;
    if (id == CL_CLASS(CTFProjectile_Cleaver))
        return *cleavers;
    if (id == CL_CLASS(CTFGrapplingHook))
        return false;
    return *proj_default;
}
