...
bool InitSchema(const char *fileName, const char *pathID, CUtlVector<CUtlString> *pVecErrors /* = NULL */)
{
    ...
    if (!schema || !BInitTextBuffer)
    {
        logging::Info("Item schema init path missing");
        return false;
    }
...
void Shutdown()
{
    ...
    */
    if (ScoreboardColoring::patch_scoreboardcolor1)
        ScoreboardColoring::patch_scoreboardcolor1->Shutdown();
    if (ScoreboardColoring::patch_scoreboardcolor2)
...
{
    auto *enable_wait = g_ICvar ? g_ICvar->FindVar("sv_allow_wait_command") : nullptr;
    if (enable_wait)
        enable_wait->SetValue(after ? 1 : 0);
}
