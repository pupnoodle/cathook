...
void apply_killstreaks()
{
    ...
    ClientClass *cc         = EntClientClass(resource);
    if (!ent || !cc || cc->m_ClassID != RCC_PLAYERRESOURCE)
        return;

    if (netvar.m_nStreaks_Resource)
    {
        int *streaks_resource = (int *) ((uintptr_t) resource + netvar.m_nStreaks_Resource + 4 * g_IEngine->GetLocalPlayer());
    ...
    if (netvar.m_nStreaks_Player)
    {
        int *streaks_player = (int *) ((uintptr_t) ent + netvar.m_nStreaks_Player);
