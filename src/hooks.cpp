...
VMTHook::VMTHook()
{
    static_assert(ptr_size == 8, "Pointer size must be QWORD.");
...
void VMTHook::Set(ptr_t inst, uint32_t offset)
{
    ...
    logging::Info("Hooking vtable %p with %d methods", vtable_original, mc);
...
void VMTHook::Release()
{
    ...
    {
        logging::Info("Un-hooking %p (vtable @ %p)", vtable_ptr, *vtable_ptr);
...
{
    logging::Info("Hooking method %d of vtable %p, replacing %p with %p", idx, vtable_original, GetMethod(idx), func);
    if (backup)
