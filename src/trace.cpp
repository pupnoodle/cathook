...
bool trace::FilterDefault::ShouldHitEntity(IHandleEntity *handle, int mask)
{
    ...
    clazz  = EntClientClass(entity);
    if (!clazz)
        return false;
    /* Ignore invisible entities that we don't wanna hit */
    int cid = clazz->m_ClassID;
    if (class_is(cid, CL_CLASS(CFuncRespawnRoomVisualizer), CL_CLASS(CTFMedigunShield), CL_CLASS(CFuncAreaPortalWindow)))
        return false;
    if (cid == CL_CLASS(CTFPlayer))
    {
        if (m_pSelf)
        {
...
bool trace::FilterNoPlayer::ShouldHitEntity(IHandleEntity *handle, int mask)
{
    ...
    /* Ignore invisible entities that we don't wanna hit */
    if (class_is(clazz->m_ClassID, CL_CLASS(CTFPlayerResource), CL_CLASS(CTFPlayer), CL_CLASS(CFuncRespawnRoomVisualizer), CL_CLASS(CTFMedigunShield), CL_CLASS(CFuncAreaPortalWindow)))
        return false;
    /* Do not hit yourself. Idiot. */
    if (entity == m_pSelf)
        return false;
...
bool trace::FilterNoEntity::ShouldHitEntity(IHandleEntity *handle, int mask)
{
    ...
    // Hit doors, carts, etc
    if (class_is(clazz->m_ClassID, CL_CLASS(CWorld), CL_CLASS(CPhysicsProp), CL_CLASS(CDynamicProp), CL_CLASS(CBaseDoor), CL_CLASS(CBaseEntity)))
        return true;
    return false;
}
