#ifndef SEED_PREDICTION_HPP
#define SEED_PREDICTION_HPP
...
inline void ask() {
  ...
  engine->client_cmd_unrestricted("playerperf");
  waiting = true;
...
inline bool on_text_msg(bf_read* message) {
  ...
  const int size = std::max(message->data_bytes, 0);
  if (size <= 1) {
    return false;
  }
  std::string text(reinterpret_cast<const char*>(bytes), static_cast<std::size_t>(size));
  for (char& ch : text) {
    if (ch == '\0') {
      ch = ' ';
    }
