...
{
    return class_is(class_id, CL_CLASS(CTFRocketLauncher), CL_CLASS(CTFParticleCannon), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFRocketLauncher_Mortar), CL_CLASS(CTFRocketLauncher_DirectHit));
}
void BeginConVars()
{
...
{
    if (class_is(class_id, CL_CLASS(CTFGrenadePipebombProjectile), CL_CLASS(CTFProjectile_Cleaver), CL_CLASS(CTFProjectile_Jar), CL_CLASS(CTFProjectile_JarMilk)))
        return 1.0f;
    if (class_id == CL_CLASS(CTFProjectile_Arrow))
        return 2599.0f <= x_speed ? 0.1f : 0.5f;
    if (class_id == CL_CLASS(CTFProjectile_Flare))
        return 0.25f;
    if (class_id == CL_CLASS(CTFProjectile_HealingBolt))
        return 0.2f;
    if (class_is(class_id, CL_CLASS(CTFProjectile_Rocket), CL_CLASS(CTFProjectile_SentryRocket), CL_CLASS(CTFProjectile_EnergyBall), CL_CLASS(CTFProjectile_EnergyRing), CL_CLASS(CTFProjectile_GrapplingHook), CL_CLASS(CTFProjectile_BallOfFire)))
        return 0.0f;
    return 0.3f;
}
weaponmode GetWeaponMode(CachedEntity *ent)
{
    ...
    if (class_is(classid, CL_CLASS(CTFRocketLauncher_DirectHit), CL_CLASS(CTFRocketLauncher), CL_CLASS(CTFGrenadeLauncher), CL_CLASS(CTFPipebombLauncher), CL_CLASS(CTFCompoundBow), CL_CLASS(CTFBat_Wood), CL_CLASS(CTFBat_Giftwrap), CL_CLASS(CTFFlareGun), CL_CLASS(CTFFlareGun_Revenge), CL_CLASS(CTFSyringeGun), CL_CLASS(CTFCrossbow), CL_CLASS(CTFShotgunBuildingRescue), CL_CLASS(CTFDRGPomson), CL_CLASS(CTFWeaponFlameBall), CL_CLASS(CTFRaygun), CL_CLASS(CTFGrapplingHook), CL_CLASS(CTFParticleCannon), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFCannon), CL_CLASS(CTFMechanicalArm)))
...
bool GetProjectileData(CachedEntity *weapon, float &speed, float &gravity, float &start_velocity)
{
    ...
    int id = weapon->m_iClassID();
    if (id == CL_CLASS(CTFRocketLauncher_DirectHit))
        rspeed = 1980.0f;
    else if (class_is(id, CL_CLASS(CTFParticleCannon), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFRocketLauncher)))
    {
        rspeed = 1100.0f;
        if (CE_INT(weapon, netvar.iItemDefinitionIndex) == 414)
            rspeed = 1540.0f;
    }
    else if (id == CL_CLASS(CTFCannon))
    {
        rspeed       = 1453.9f;
        rgrav        = 1.0f;
        rinitial_vel = 200.0f;
    }
    else if (id == CL_CLASS(CTFGrenadeLauncher))
    {
        rspeed       = 1216.6f;
        rgrav        = 1.0f;
        rinitial_vel = 200.0f;
    ...
    else if (id == CL_CLASS(CTFPipebombLauncher))
    {
        ...
        rgrav        = 1.0f;
        rinitial_vel = 200.0f;
        if (CE_INT(weapon, netvar.iItemDefinitionIndex) == 1150)
            rspeed = RemapValClamped(chargetime, 0.0f, 4.0f, 930.88, 2409.2);
    }
    else if (id == CL_CLASS(CTFCompoundBow))
    {
        float chargetime = g_GlobalVars->curtime - CE_FLOAT(weapon, netvar.flChargeBeginTime);
    ...
    else if (class_is(id, CL_CLASS(CTFBat_Giftwrap), CL_CLASS(CTFBat_Wood)))
    {
        rspeed = 3000.0f;
        rgrav  = 0.5f;
    }
    else if (class_is(id, CL_CLASS(CTFFlareGun), CL_CLASS(CTFFlareGun_Revenge)))
    {
        rspeed = 2000.0f;
        rgrav  = 0.25f;
    }
    else if (id == CL_CLASS(CTFSyringeGun))
    {
        rgrav  = 0.3f;
        rspeed = 1000.0f;
    }
    else if (class_is(id, CL_CLASS(CTFCrossbow), CL_CLASS(CTFShotgunBuildingRescue)))
    {
        rgrav  = 0.2f;
        rspeed = 2400.0f;
    }
    else if (class_is(id, CL_CLASS(CTFDRGPomson), CL_CLASS(CTFRaygun)))
        rspeed = 1200.0f;
    else if (class_is(id, CL_CLASS(CTFWeaponFlameBall), CL_CLASS(CTFCleaver)))
        rspeed = 3000.0f;
    else if (id == CL_CLASS(CTFGrapplingHook))
        rspeed = 1500.0f;
    else if (id == CL_CLASS(CTFJarMilk))
        rspeed = 1019.9f;
    else if (id == CL_CLASS(CTFJar))
        rspeed = 1017.9f;
    else if (id == CL_CLASS(CTFJarGas))
        rspeed = 2009.2f;
...
{
    int id = LOCAL_W->m_iClassID();
    if (class_is(id, CL_CLASS(CTFRocketLauncher_DirectHit), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFRocketLauncher_Mortar), CL_CLASS(CTFRocketLauncher), CL_CLASS(CTFParticleCannon), CL_CLASS(CTFGrenadeLauncher), CL_CLASS(CTFPipebombLauncher)))
        return HasCondition<TFCond_UberBlastResist>(player);
    if (class_is(id, CL_CLASS(CTFCompoundBow), CL_CLASS(CTFSyringeGun), CL_CLASS(CTFCrossbow), CL_CLASS(CTFShotgunBuildingRescue), CL_CLASS(CTFDRGPomson), CL_CLASS(CTFRaygun)))
        return HasCondition<TFCond_UberBulletResist>(player);
    if (class_is(id, CL_CLASS(CTFWeaponFlameBall), CL_CLASS(CTFFlareGun), CL_CLASS(CTFFlareGun_Revenge), CL_CLASS(CTFFlameRocket), CL_CLASS(CTFFlameThrower)))
        return HasCondition<TFCond_UberFireResist>(player);
    return g_pLocalPlayer->weapon_mode == weaponmode::weapon_hitscan && HasCondition<TFCond_UberBulletResist>(player);
}
...
float ATTRIB_HOOK_FLOAT(float base_value, const char *search_string, IClientEntity *ent, void *buffer, bool is_global_const_string)
{
    ...
    static uintptr_t AttribHookFloat = gSignatures.GetClientSignature(sigs::attribute_hook_value_float);
...
bool IsEntityVisiblePenetration(CachedEntity *entity, int hb)
{
    ...
    if (trace_visible.m_pEnt)
    {
        ...
        if (ent)
        {
            ClientClass *hit_cc = EntClientClass(ent);
            if (hit_cc && hit_cc->m_ClassID == RCC_PLAYER)
            {
...
{
    auto *verified = GetVerifiedCmds(g_IInput);
    if (!cmd || !verified)
        return;
    CRC32_t crc = GetChecksum(cmd);
    if (crc != verified[sequence_nr % VERIFIED_CMD_SIZE].m_crc)
        *cmd = verified[sequence_nr % VERIFIED_CMD_SIZE].m_cmd;
}
                            ...
                            []()
                            {
                                ...
                                for (auto const &ent : entity_cache::valid_ents)
                                {
                                    ...
                                    if (cc && cc->m_pNetworkName)
                                        logging::Info("%s", cc->m_pNetworkName);
                                }
                            });
...
Vector getShootPos(Vector angle)
{
    ...
    int id = LOCAL_W->m_iClassID();
    if (class_is(id, CL_CLASS(CTFRocketLauncher), CL_CLASS(CTFRocketLauncher_Mortar), CL_CLASS(CTFRocketLauncher_AirStrike), CL_CLASS(CTFRocketLauncher_DirectHit), CL_CLASS(CTFFlareGun), CL_CLASS(CTFFlareGun_Revenge), CL_CLASS(CTFDRGPomson)))
    {
    ...
    else if (class_is(id, CL_CLASS(CTFPipebombLauncher), CL_CLASS(CTFGrenadeLauncher), CL_CLASS(CTFCannon)))
