...
inline T GetVar(IClientEntity* ent, unsigned int offset) {
    ...
    return *(reinterpret_cast<T*>((uintptr_t)ent + offset));
}

template<typename T>
void SetVar(IClientEntity* ent, unsigned int offset, T value) {
    *(reinterpret_cast<T*>((uintptr_t)ent + offset)) = value;
}*/
...
public:
    ...
    offset_t flLastCritCheckTime;
    offset_t bPlayerDominated;
    ...
    offset_t m_nCurrency;
    offset_t m_bUsingActionSlot;
    offset_t m_hConstraintEntity;
    offset_t m_pCurrentCommand;
    ...
    offset_t m_Shared;
    offset_t m_iCritMult;
    offset_t m_iRoundState;
    offset_t m_iWinningTeam;
    offset_t m_bPlayingMannVsMachine;
    offset_t m_halloweenScenario;
    offset_t m_bIsUsingSpells;
};
